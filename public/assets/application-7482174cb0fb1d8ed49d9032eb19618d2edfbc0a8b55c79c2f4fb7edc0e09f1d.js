	...
	geocoder.geocode({ 'latLng': latLng }, function(results, status) {		
		if(status == google.maps.GeocoderStatus.OK) {
			if(results[0]) {
				...
				var infoWindowYourPositionContent = '<a class="add-spot-button" href=/spots/new?latitude=' + yourLatitude + '&longitude=' + yourLongitude + '&address=' + addressCorrection + '>Add spot here</a>';
...
$('.hearts').on('click', function() {		
	...
	$.ajax({
		...
		data: { trickRating },
		dataType: 'json',
		success: function() { console.log('OK2'); },
		error: function() { console.log('KO2'); }
	});
});
